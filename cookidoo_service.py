...
class CookidooService:
    ...
    async def create_custom_recipe_with_tts(
        self,
        name: str,
        ingredients: list[str],
        steps: list,  # list of StepWithTTS or str
        servings: int = 4,
        prep_time: int = 30,
        total_time: int = 60,
        hints: Optional[list[str]] = None,
        ...
        """
        Create a custom recipe with TTS (Thermomix Time Settings) annotations.
        
        Args:
            name: Recipe name
            ingredients: List of ingredient descriptions
            steps: List of StepWithTTS objects or strings
            servings: Number of servings (default: 4)
            prep_time: Preparation time in minutes (default: 30)
            total_time: Total cooking time in minutes (default: 60)
            hints: Optional list of hints/tips for the recipe
        ...
        try:
            ...
            # Convert steps to StepWithTTS if needed
            processed_steps = []
            for step in steps:
                if isinstance(step, StepWithTTS):
                    processed_steps.append(step)
                else:
                    # Create StepWithTTS from string
                    processed_steps.append(StepWithTTS.from_string(str(step)))
            ...
            async with api_session.post(
                ...
                if response.status != 200:
                    error_text = await response.text()
                    raise Exception(f"Failed to create recipe. Status: {response.status}, Error: {error_text}")
                
                result = await response.json()
                recipe_id = result.get("recipeId")
                if not recipe_id:
                    raise Exception("No recipe ID returned")
            
            # Step 2: Update with full content including TTS annotations
            update_url = f"{base_url}/created-recipes/{locale}/{recipe_id}"
            ...
            update_data = {
                ...
                "ingredients": [{"type": "INGREDIENT", "text": ing} for ing in ingredients],
                "instructions": [step.to_dict() for step in processed_steps],
                "hints": "\n".join(hints) if hints and isinstance(hints, list) else (hints if hints else ""),
                "workStatus": "PRIVATE",
                "recipeMetadata": {
                    "requiresAnnotationsCheck": False
            ...
            async with api_session.patch(update_url, json=update_data, headers=headers) as response:
                if response.status not in [200, 204]:
                    error_text = await response.text()
                    raise Exception(f"Failed to update recipe: {error_text}")
            
            return recipe_id
            
        except Exception as e:
            raise Exception(f"Failed to create custom recipe with TTS: {str(e)}") from e
